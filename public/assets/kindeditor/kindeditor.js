...
function _elementVal(knode, val) {
	if (knode.hasVal()) {
        ...
        if(self.placeholder != ""){
            if(/^\s*<\w*\s*\w*\=\"\w*\"\s*\w*\=\"\w*\:\s*\#\d*\;\s*\w*\-\w*\:\s*\w*\;\"\>[\u4e00-\u9fa5]*[\，]*[\（]*[\u4e00-\u9fa5]*[\：]*[\u4e00-\u9fa5]*[\。]*[\）]*<\/\w*\>\s*$/.test(val)){
                return knode.val("");
            }
        }
...
//					}
	...
	isEmpty : function() {
        ...
        if(self.placeholder == ""){
            ...
            return (_trim(this.text().replace(/\r\n|\n|\r/, '')) === '' || /^\s*<\w*\s*\w*\=\"\w*\"\s*\w*\=\"\w*\:\s*\#\d*\;\s*\w*\-\w*\:\s*\w*\;\"\>[\u4e00-\u9fa5]*[\，]*[\（]*[\u4e00-\u9fa5]*[\：]*[\u4e00-\u9fa5]*[\。]*[\）]*<\/\w*\>\s*$/.test(self.edit.html()));
        }
	},
	isDirty : function() {
...
//    options.emotionsBasePath = _undef(options.emotionsBasePath,'http://forge.trustie.net') // TODO
    options.emotionsBasePath = _undef(options.emotionsBasePath,'') // 使用本地的emotion图片
...
_plugin('core', function(K) {
	...
	self.uploadButton = K.uploadbutton({
        ...
        url:K.addParam('/api/attachments.json'),
        // url:K.addParam( 'http://testbdweb.educoder.net' + '/kindeditor/upload', 'dir=image'),
		
        afterUpload : function(data) {
            if (data.id || data.error === 0) {
				data.url =  '/api/attachments/' + data.id
...
//                alert("仅支持如下图片格式:bmp,jpeg,jpg,png,gif,BMP,JPEG,JPG,PNG,GIF");
                long_notice_box("仅支持如下图片格式:bmp,jpeg,jpg,png,gif!");
	...
	self.uploadButton.fileBox.change(function(e) {
		// 检查如果不是图片格式
		var filePath = e.event.currentTarget.value;
		var dotIndex = filePath.lastIndexOf('.')
		var fileType = filePath.substring(dotIndex + 1)
		if (['bmp', 'jpeg', 'jpg', 'png', 'gif'].indexOf(fileType.toLowerCase()) == -1) {
			long_notice_box("仅支持如下图片格式:bmp,jpeg,jpg,png,gif!");
			return;
		}
