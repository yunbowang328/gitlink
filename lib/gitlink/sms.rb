...
module  Gitlink
  module Sms
    ...
    def self.sendYunpian(mobile, code, send_type, name, user_name, result)
      ...
      params['text'] = ""
      if send_type.nil?
        params['text'] = "【GitLink(确实开源)】" + code + "（手机验证码），有效期为10分钟。如非本人操作，请忽略。"
      elsif send_type == 'competition_start'
        params['text'] = "【GitLink(确实开源)】亲爱的#{user_name}，你参与的#{name}将于#{result}开始，请及时参赛"
      elsif send_type == "teacher_register"
        params['mobile'] = EduSetting.get('teacher_register_phone') || '17680641960'
        params['text'] = "【GitLink(确实开源)】亲爱的#{user_name}，有新的老师#{name}注册啦，请尽快处理"
        ...
        params['text'] = "【GitLink(确实开源)】亲爱的#{name}，有新的#{send_type == 'apply_pro_certification'?'职业':'实名'}认证申请，请尽快处理"
        ...
        params['text'] = "【GitLink(确实开源)】亲爱的#{name}，有新的#{send_type == 'publish_subject'?'实训路径':(send_type == 'publish_shixun' ? '实训' : (send_type == 'discuss' ? '实训评论':'试用'))}申请发布，请尽快处理"
      elsif send_type == 'join_course_multi_role'
        params['text'] = "【GitLink(确实开源)】亲爱的#{user_name}，您的课堂#{name}有助教或者教师申请加入，请尽快审核"
      elsif send_type == 'applied_project_info'
        params['text'] = "【GitLink(确实开源)】亲爱的#{user_name}，您的项目#{name}有成员申请加入，请尽快审核"
      end
